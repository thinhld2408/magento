...
class SM_MegaMenu_Adminhtml_MenuController extends Mage_Adminhtml_Controller_Action
{
    ...
    protected function _beforeSave($data){
        ...
        if($data['item_type'] == 'category_link'){
            if($data['type_view'] == "tree"){
                $data['item_list_id'] = $data["category_id"];
            }else{
                $data['item_list_id'] = implode(",",$data['item_category']);
            }

        }
        if($data['item_type'] == 'block_link'){
            $data['item_list_id'] = implode(",",$data['cms_block']);
        }
        if($data['item_type'] == 'custom_link'){
